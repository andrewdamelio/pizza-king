export const MOVEMENT = '@@MyApp/worm/MOVEMENT';
export const SAVE_BOX_INFO = '@@MyApp/worm/SAVE_BOX_INFO';
export const SHRINK = '@@MyApp/worm/SHRINK';
export const GROW = '@@MyApp/worm/GROW';
export const UPDATE_INDEX = '@@MyApp/history/UPDATE_INDEX';
export const SET_REPLAY_MODE = '@@MyApp/history/SET_REPLAY_MODE';
export const RESET_GAME = '@@MyApp/history/RESET_GAME';
export const SPEED_UP = '@@MyApp/history/SPEED_UP';
export const CREATE_PIZZA = '@@MyApp/pizza/CREATE_PIZZA';
export const UPDATE_PIZZA = '@@MyApp/pizza/UPDATE_PIZZA';
export const ADD_PLAYER = '@@MyApp/game/ADD_PLAYER';
export const REMOVE_PLAYER = '@@MyApp/game/REMOVE_PLAYER';
export const TOGGLE_VICTORY = '@@MyApp/game/TOGGLE_VICTORY';
export const JOIN_GAME = '@@MyApp/game/JOIN_GAME';
export const ADD_MSG = '@@MyApp/chat/ADD_MSG';
export const UPDATE_LOG = '@@MyApp/chat/UPDATE_LOG';

